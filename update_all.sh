...
wsk -i action update class/cdAction --docker $RUNTIME_NAME --timeout 300000  -p ALIAS DKB -p CHUNK_SIZE 3 -p REDIS_HOST ${REDIS_HOST} --memory 512 -p OPERATION cd -p RUNTIME ${cd_runtime} classAction.zip
wsk -i action update class/tpAction --docker $RUNTIME_NAME --timeout 300000  -p ALIAS DKB -p CHUNK_SIZE 3 -p REDIS_HOST ${REDIS_HOST} --memory 512 -p OPERATION tp -p RUNTIME ${tp_runtime} classAction.zip
